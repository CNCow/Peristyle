...
// ── Update Checker ─────────────────────────────────────────────────────────
const GITHUB_MANIFEST_URL = 'https://raw.githubusercontent.com/CNCow/Peristyle/main/manifest.json';
const GITHUB_ZIP_URL      = 'https://github.com/CNCow/Peristyle/archive/refs/heads/main.zip';
const GITHUB_REPO_URL     = 'https://github.com/CNCow/Peristyle';
...
function importConfig(file) {
  ...
  reader.onload = e => {
    try {
      ...
      alert('Could not read config.json — make sure it is a valid Peristyle config file.');
    }
  };
...
function applyTitleVisibility() {
  const el = $('site-title');
  el.textContent = state.title || 'Peristyle';
  el.classList.toggle('visible', !!state.showTitle);
  document.title = state.title || 'Peristyle';
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  $('title-save').addEventListener('click', () => {
    state.title = $('title-input').value.trim() || 'Peristyle';
